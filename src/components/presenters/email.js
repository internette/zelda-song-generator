...
const EmailPresenter = (props) => {
  ...
  if(props.visible_text === 'email-instructions'){
    email_modal_text = <div><h3>E-mail This Song</h3><div><p>Enter the e-mail where you would like the download URL for the song sent below.</p><p><label htmlFor="email_address">E-mail Address</label><p id="error">You must provide a valid e-mail address</p><input id="email_address" onChange={(e)=> { return props.updateEmail(e.target.value)}} /></p><button className="black button" onClick={()=>{ props.sendSong(props.notes, props.song_name, props.instrument, props.email_address) }} id="submit-send-email">Send Me My Song</button></div><audio id="hey-listen"><source type="audio/mp3" src={mp3_file}/>></audio></div>
  } else {
