...
const InstrumentsPresenter = (props) => {
  ...
  if(props.window_width > 768){
    ...
    content = <Scrollbars renderTrackVertical={() => <div/>} renderTrackHorizontal={({ style, ...scrollbar_props }) => <div {...scrollbar_props} className="instruments-list-scrollbar-track" style={{...style, height: '12px', width: '96%', left: '2%', right: 'auto', top: 'auto', bottom: '3px' }}/>} renderThumbHorizontal={({ style, ...scrollbar_props }) => <div {...scrollbar_props} style={{ ...style }}/>} style={{height: '8.5rem', width: '100%', whiteSpace: 'nowrap'}}><div className="instrument-list">{ props.instruments.map((instrument, index)=> {return <Instrument key={index} {...instrument}/>}) }</div>
  </Scrollbars>
  }
  return (
    <div id="instruments">
